...
before(async () => {
  ...
  // Generar token válido con rol admin
  token = jwt.sign(
...
describe("Módulo Adoption", () => {
  ...
  it("GET /api/adoptions debería devolver array vacío inicialmente", async () => {
  ...
  it("POST /api/adoptions/:uid/:pid crea adopción correctamente", async () => {
    ...
    expect(res.body.adoption.user).to.equal(userId);
    expect(res.body.adoption.pet).to.equal(petId);
  });
